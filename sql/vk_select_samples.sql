...
,
  MONTHNAME(created_at) AS month_name -- , MONTH(created_at) AS month_num -- если заходим вывести номер месяца (вспомогательно) 
FROM media
GROUP BY month_name -- order by month(created_at) -- упорядочим по месяцам
...
-- фишка MySQL, в MS SQL Server такой команды нет
